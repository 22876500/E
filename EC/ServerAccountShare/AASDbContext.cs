...
namespace Server
{
    ...
    public class AASTraderLimitConfiguration : EntityTypeConfiguration<额度分配>
    {
        ...
        {
            Property(t => t.手续费率).HasPrecision(18, 6);
            Property(t => t.交易额度).HasPrecision(18, 6);
        }
    ...
    public class AASOrderConfig : EntityTypeConfiguration<已发委托>
    {
        ...
        {
            Property(t => t.成交价格).HasPrecision(20, 10);
            Property(t => t.成交数量).HasPrecision(18, 6);

            Property(t => t.委托价格).HasPrecision(20, 10);
            Property(t => t.委托数量).HasPrecision(18, 6);

            Property(t => t.撤单数量).HasPrecision(18, 6);
        }
    }

    //可用资金
    public class AASCashConfig : EntityTypeConfiguration<可用资金>
    {
        public AASCashConfig()
        {
            Property(t => t.可用数量).HasPrecision(20, 10);
        }
    }

    //订单
    public class AASTradeConfig : EntityTypeConfiguration<订单>
    {
        public AASTradeConfig()
        {
            Property(t => t.当前价位).HasPrecision(20, 10);
            Property(t => t.开仓价位).HasPrecision(20, 10);
            Property(t => t.平仓价位).HasPrecision(20, 10);

            Property(t => t.已开数量).HasPrecision(18, 6);
            Property(t => t.已开金额).HasPrecision(18, 6);
            Property(t => t.已平数量).HasPrecision(18, 6);
            Property(t => t.已平金额).HasPrecision(18, 6);
            
        }
    }

    //已处理成交， 已平仓订单 两表未进行额度修正

    class AASDbContext : DbContext
    {
        public AASDbContext()
            : base("Data Source=.\\;Initial Catalog=ECoinServer;Integrated Security=True;MultipleActiveResultSets=True;Application Name=EntityFrameworkMUE")
        {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Configurations.Add(new AASCashConfig());
            modelBuilder.Configurations.Add(new AASTradeConfig());
        ...
        public DbSet<电子币帐户> 电子币帐户 { get; set; }

        public DbSet<可用资金> 可用资金 { get; set; }

        public DbSet<交易账户关联> 交易账户关联 { get; set; }
    }
    ...
    public class 可用资金
    {
        [Key, Column(Order = 0), MinLength(1)]
        public string 交易员 { get; set; }

        [Key, Column(Order = 1), MinLength(1)]
        public string 币种 { get; set; }

        public decimal 可用数量 { get; set; }
    }
