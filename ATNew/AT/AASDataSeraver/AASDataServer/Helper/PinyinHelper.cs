...
namespace AASDataServer.Helper
{
    public class PinyinHelper
    {
        ...
        /// 汉字转换成全拼的拼音
        /// </summary>
        /// <param name="name">汉字字符串</param>
        /// <returns>全拼字符串</returns>
        public static string ToQuanPin(string name)
        {
            Regex reg = new Regex("^[\u4e00-\u9fa5]$");//验证是否输入汉字
            byte[] arr = new byte[2];
            string pystr = "";
            int asc = 0, M1 = 0, M2 = 0;
            char[] mChar = name.ToCharArray();//获取汉字对应的字符数组
        ...
        /// </summary>
        /// <param name="name">汉字字符串</param>
        /// <returns>简拼字符串</returns>
        public static string ToJianPin(string name)
        {
            Regex reg = new Regex("^[\u4e00-\u9fa5]$");//验证是否输入汉字
            byte[] arr = new byte[2];
            string pystr = "";
                        ...
                        switch (asc)
                        {
                            ...
                            default:
                                for (int i = (getValue.Length - 1); i >= 0; i--)
                                {
                                    if (getValue[i] <= asc)//判断汉字的拼音区编码是否在指定范围内
                                    {
                                        pystr += getName[i].Substring(0, 1);//如果不超出范围则获取对应的拼音
                                        break;
                                    }
                                }
                                break;
