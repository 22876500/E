...
namespace AASServer
{
    ...
    class AASService
    {
        ...
        public void AddQSAccount(bool Enabled, string Name, string QS, string Type, string 交易服务器, string Version, short YybID, string Account, string TradeAccount, string JyPassword, string TxPassword, string SHGDDM, string SZGDDM, int 查询间隔时间, decimal 手续费率, int 买入方式, int 卖出方式)
        {
            ...
            Program.db.券商帐户.AddQSAccount(Enabled, Name, QS, Type, 交易服务器, Version, YybID, Account, TradeAccount, JyPassword, TxPassword, SHGDDM, SZGDDM, 查询间隔时间, 手续费率, 买入方式, 卖出方式);
        }
        ...
        {
            Program.db.券商帐户.UpdateQSAccount(Name, 交易服务器, Version, 交易密码, 通讯密码, 查询间隔时间, 手续费率, 买入方式, 卖出方式);
        }
        ...
        {
            Program.db.额度分配.DeleteTradeLimit(交易员, 证券代码, 组合号);
        }
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public DbDataSet.额度分配DataTable QueryTraderLimits(string 交易员)
        {
            return Program.db.额度分配.QueryTraderLimits(交易员);
        }
        #endregion
        ...
        public string SendOrder(string UserName, string 证券代码, int 买卖方向, decimal 委托数量, decimal 委托价格)
        {
            ...
            try
            {
                ...
                var limits = Program.db.额度分配.GetAllLimit(UserName, 证券代码);
                ...
                else
                {
                    lock (Program.SendOrderObjDict[UserName])
                    {
                        ...
                        {
                            result = SendOrderMain(UserName, 证券代码, 买卖方向, 委托数量, 委托价格, limits.First(), AASUser1);
                        }
                    }
                }
        ...
        private static string SendOrderMain(string UserName, string 证券代码, int 买卖方向, decimal 委托数量, decimal 委托价格, DbDataSet.额度分配Row limit, DbDataSet.平台用户Row AASUser1)
        {
            DbDataSet.额度分配Row TradeLimit1 = limit;
        ...
        private static string SendMultyAccountOrder(string UserName, string 证券代码, int 买卖方向, decimal 委托数量, decimal 委托价格, DbDataSet.平台用户Row AASUser1)
        {
            ...
            List<KeyValuePair<bool, string>> orderResultList = new List<KeyValuePair<bool, string>>();
            ...
            foreach (var item in orderedLimit)
            {
                ...
                {
                    SendGroupOrder(UserName, 证券代码, 买卖方向, wtQtyLeft, 委托价格, zqName, TradeLimitItem, out result, out errInfo);

                    if (errInfo.Length > 0)
                    {
                        orderResultList.Add(new KeyValuePair<bool, string>(false, errInfo));
                    }
                    else
                    {
                        orderResultList.Add(new KeyValuePair<bool, string>(true, result));
                    }
                    break;
                }
                else
                {
                    SendGroupOrder(UserName, 证券代码, 买卖方向, item.Value, 委托价格, zqName, TradeLimitItem, out result, out errInfo);
                    if (errInfo.Length > 0)
                    {
                        orderResultList.Add(new KeyValuePair<bool, string>(false, errInfo));
                    }
        ...
        private static string SendGroupOrder(string UserName, string 证券代码, int 买卖方向, decimal 委托数量, decimal 委托价格, string zqName, AASServer.DbDataSet.额度分配Row TradeLimit1, out string 委托编号, out string ErrInfo)
        {
            OrderCacheEntity orderCacheObj = new OrderCacheEntity()
            {
                Category = 买卖方向,
        ...
        public string SendAyersOrder(string trader, string 证券代码, string 证券名称, int 买卖方向, int 委托数量, decimal 委托价格)
        {
            ...
            lock (AASUser1)
            {
                DbDataSet.额度分配Row TradeLimit1 = Program.db.额度分配.Get额度分配(trader, "ayers", 证券代码);
        ...
        {
            return Program.db.可用仓位.LockPosition(组合号, 证券代码, 申请数量, 缩写, 交易员);
        }
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public DbDataSet.可用仓位DataTable QueryTotalPosition()
        {
            return Program.db.可用仓位.QueryAll();
        }
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public List<DbDataSet.可用仓位DataTable.可用仓位Extend> GetPositonLockedAll()
        {
            return Program.db.可用仓位.GetPositonLockAll();
        }
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public DbDataSet.额度分配DataTable GetPositionLockedDetail(string 组合号, string 证券代码)
        {
            return Program.db.可用仓位.GetPositionLockDetail(组合号, 证券代码);
        }
        #endregion
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public bool Add可用仓位(string 组合号, string 证券代码, string 证券名称, decimal 总仓位)
        {
            return Program.db.可用仓位.Add(组合号, 证券代码, 证券名称, 总仓位);
        }

        [OperationContract]
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public string AddMuti可用仓位(DbDataSet.可用仓位DataTable dt, bool isCoverExists)
        {
            StringBuilder errMsg = new StringBuilder(256);
            List<string> errList = new List<string>(128);
            foreach (DataRow item in dt.Rows)
            {
                if (isCoverExists)
                {
                    Program.db.可用仓位.AddOrUpdate(item["组合号"].ToString(), item["证券代码"].ToString(), item["证券名称"].ToString(), decimal.Parse(item["总仓位"].ToString()));
                }
                else
                {
                    if (!Program.db.可用仓位.Add(item["组合号"].ToString(), item["证券代码"].ToString(), item["证券名称"].ToString(), decimal.Parse(item["总仓位"].ToString())))
                    {
                        errList.Add(string.Format("券商账户{0},证券代码{1}", item["组合号"].ToString(), item["证券代码"].ToString()));
                    }
                }
            ...
            {
                errMsg.Append("0|");
                errMsg.Append("部分仓位添加失败，已存在对应仓位数据,").Append(string.Join("、", errList));
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public void DeleteAll可用仓位()
        {
            Program.db.可用仓位.DeleteAll();
        }

        [OperationContract]
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public void Delete可用仓位(string 组合号, string 证券代码)
        {
            Program.db.可用仓位.Delete(组合号, 证券代码);
        }
        #endregion
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public string Update可用仓位(string 组合号, string 证券代码, string 证券名称, decimal 总仓位)
        {
            return Program.db.可用仓位.Update(组合号, 证券代码, 证券名称, 总仓位);
        }
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public string AddTraderAccount(string trader, string account)
        {
            return Program.db.组合号交易员关联.Add(account, trader);
        }
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public DbDataSet.组合号交易员关联DataTable QueryAllTraderAccounts()
        {
            return Program.db.组合号交易员关联.QueryAll();
        }

        [OperationContract]
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public void DeleteRelationByAccount(string 组合号)
        {
            Program.db.组合号交易员关联.DeleteByAccount(组合号);
        }

        [OperationContract]
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public void DeleteRelationByTrader(string trader)
        {
            Program.db.组合号交易员关联.DeleteByTrader(trader);
        }

        [OperationContract]
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public void DeleteTraderAccountRelation(string account, string trader)
        {
            Program.db.组合号交易员关联.Delete(account, trader);
        }
        #endregion
        ...
        {
            return Program.db.平台用户.QueryTraderNameList();
        }

        [OperationContract]
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public List<string> QueryQSNameList()
        {
            var normalAccounts = Program.db.券商帐户.QueryQsNameList();
            var hsAccounts = Program.db.恒生帐户.QueryQsHsNameList();
            normalAccounts.AddRange(hsAccounts);
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "超级风控员")]
        [PrincipalPermission(SecurityAction.Demand, Role = "普通风控员")]
        public DbDataSet.订单DataTable QueryAll订单()
        {
            var fkName = OperationContext.Current.ServiceSecurityContext.PrimaryIdentity.Name;
            return Program.db.订单.Query订单BelongFK(fkName); ;
        }
        ...
        public AASServer.JyDataSet.业绩统计DataTable Query业绩BelongFK(string UserName, DateTime StartDate, DateTime EndDate)
        {
            ...
            try
            {
                ...
                foreach (AASServer.JyDataSet.业绩统计Row 业绩统计Row1 in 业绩统计DataTable1)
                {
                    ...
                    {
                        业绩统计Row1.毛利 = 业绩统计Row1.卖出金额 - 业绩统计Row1.买入金额;
    ...
    public interface IClient
    {
        ...
        [OperationContract(IsOneWay = true)]
        void 可用仓位DataTableChange(AASServer.DbDataSet.可用仓位DataTable TableChanged);

        [OperationContract(IsOneWay = true)]
        void 可用仓位Change(string AccountName, string StockID, decimal qtyLimit);
