...
namespace AASServer
{
    ...
    public partial class DbDataSet
    {
        ...
        partial class 恒生帐户DataTable
        {
            ...
            public List<string> QueryQsHsNameList()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    return this.Where(_ => _.启用).Select(_=> _.名称).ToList();
                }
            }
        }
        ...
        partial class 券商帐户DataTable
        {
                ...
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    ...
                    using (AASDbContext db = new AASDbContext())
                    {
                        foreach (券商帐户 券商帐户1 in db.券商帐户)
                        {
                            ...
                            券商帐户Row1.买入方式 = (int)券商帐户1.买入方式;
                            券商帐户Row1.卖出方式 = (int)券商帐户1.卖出方式;
                            券商帐户Row1.手续费率 = 券商帐户1.手续费率;
                            券商帐户Row1.买入方式 = (int)券商帐户1.买入方式;
                            券商帐户Row1.卖出方式 = (int)券商帐户1.卖出方式;
                ...
                using (AASDbContext db = new AASDbContext())
                {
                    switch (e.Action)
                    {
                        case DataRowAction.Add:
                            ...
                            券商帐户1.手续费率 = e.Row.手续费率;
                            券商帐户1.买入方式 = (买模式)e.Row.买入方式;
                            券商帐户1.卖出方式 = (卖模式)e.Row.卖出方式;
                            db.券商帐户.Add(券商帐户1);
                            db.SaveChanges();
                            break;
                        case DataRowAction.Delete:
            ...
            public 券商帐户Row QueryQsAccount(string 名称)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    return this.FirstOrDefault(_ => _.名称 == 名称);
                }
            }
            ...
            public void AddQSAccount(bool Enabled, string Name, string QS, string Type, string 交易服务器, string Version, short YybID, string Account, string TradeAccount, string JyPassword, string TxPassword, string SHGDDM, string SZGDDM, int 查询间隔时间, decimal 手续费率, int 买入方式, int 卖出方式)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
            ...
            public void UpdateQSAccount(string Name, string 交易服务器, string Version, string 交易密码, string 通讯密码, int 查询间隔时间, decimal 手续费率, int 买入方式, int 卖出方式)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    AASServer.DbDataSet.券商帐户Row 券商帐户Row1 = this.First(r => r.名称 == Name);
                    券商帐户Row1.UpdateQSAccount(交易服务器, Version, 交易密码, 通讯密码, 查询间隔时间, 手续费率, 买入方式, 卖出方式);
                }
            ...
            internal List<string> QueryQsNameList()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    return this.Where(_=> _.启用).Select(_=> _.名称).ToList();
        ...
        partial class 券商帐户Row
        {
            ...
            public void UpdateQSAccount(string 交易服务器, string Version, string 交易密码, string 通讯密码, int 查询间隔时间, decimal 手续费率, int 买入方式, int 卖出方式)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    this.交易服务器 = 交易服务器;
        ...
        partial class 订单DataTable
        {
            ...
            public 订单DataTable Query订单BelongFK(string UserName)
            {
                AASServer.DbDataSet.平台用户Row 风控员 = Program.db.平台用户.Get平台用户(UserName);
                订单DataTable 订单DataTable1 = new 订单DataTable();
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    if (风控员.角色 == (int)角色.超级风控员)
                    {
                        foreach (订单Row 订单Row1 in this)
                        {
                            订单DataTable1.ImportRow(订单Row1);
                        }
                    }
                    ...
                    {
                        var traders = Program.db.风控分配.Query风控分配BelongFK(UserName);
                        if (traders.Count > 0)
                        {
                            var names = traders.Select(_ => _.交易员).Distinct().ToList();
                            var infos = this.Where(r => names.Contains(r.交易员));
                            foreach (订单Row 订单Row1 in infos)
                            {
                                订单DataTable1.ImportRow(订单Row1);
                            }
                        }
        ...
        partial class 额度分配DataTable
        {
            ...
            public void Load()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    this.PrimaryKey = new DataColumn[] { this.Columns["交易员"], this.Columns["证券代码"], this.Columns["组合号"] };
                ...
                using (AASDbContext db = new AASDbContext())
                {
                    switch (e.Action)
                    {
                        case DataRowAction.Add:
                            ...
                            break;
                        case DataRowAction.Delete:
                            额度分配1 = db.额度分配.Find(e.Row.交易员, e.Row.证券代码, e.Row.组合号);
            ...
            public 额度分配DataTable QueryTraderLimits(string 交易员)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
            ...
            public 额度分配DataTable Get额度分配(string 组合号, string 证券代码)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    AASServer.DbDataSet.额度分配DataTable 额度分配DataTable1 = new DbDataSet.额度分配DataTable();
                    foreach (var item in this)
                    {
                        if (item.组合号 == 组合号 && item.证券代码 == 证券代码)
                        {
                            额度分配DataTable1.ImportRow(item);
                        }
                    }
            ...
            public 额度分配Row Get额度分配(string 交易员, string 组合号, string 证券代码)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    AASServer.DbDataSet.额度分配DataTable 额度分配DataTable1 = new DbDataSet.额度分配DataTable();

                    额度分配Row 额度分配Row1 = this.FirstOrDefault(r => r.交易员 == 交易员 && r.证券代码 == 证券代码 && r.组合号 == 组合号);
                    if (额度分配Row1 == null)
                    {
                        return null;
                    }
            ...
            public IEnumerable<额度分配Row> GetAllLimit(string 交易员, string 证券代码)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
            ...
            public void UpdateTradeLimit(string 交易员, string 证券代码, string 组合号, byte 市场代码, string 证券名称, string 拼音缩写, 买模式 买模式1, 卖模式 卖模式1, decimal 交易额度, decimal 手续费率)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    额度分配Row TradeLimit1 = this.First(r => r.交易员 == 交易员 && r.证券代码 == 证券代码 && r.组合号 == 组合号);
            ...
            public void DeleteLimit(string 组合号, string 证券代码)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    额度分配Row[] TradeLimits = this.Where(r => r.证券代码 == 证券代码 && r.组合号 == 组合号).ToArray();
                    foreach (额度分配Row TradeLimit1 in TradeLimits)
                    {
                        Remove额度分配Row(TradeLimit1);
                    }
                }
            }

            public void DeleteTradeLimit(string 交易员, string 证券代码, string 组合号)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    AASServer.DbDataSet.额度分配Row TradeLimit1 = this.First(r => r.交易员 == 交易员 && r.证券代码 == 证券代码 && r.组合号 == 组合号);
        ...
        partial class 平台用户DataTable
        {
            ...
            public List<string> QueryTraderNameList()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    return this.Where(_=> _.角色 == (int) 角色.交易员).Select(_=> _.用户名).ToList();
                }
            }
        }
        ...
        partial class 可用仓位DataTable
        {
            ...
            public void Load()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    this.PrimaryKey = new DataColumn[] { this.Columns["证券代码"], this.Columns["组合号"] };
                    using (AASDbContext db = new AASDbContext())
                    {
                        foreach (可用仓位 可用仓位1 in db.可用仓位)
                        {
                            可用仓位Row 可用仓位Row = this.New可用仓位Row();
                            可用仓位Row.总仓位 = 可用仓位1.总仓位;
                            可用仓位Row.组合号 = 可用仓位1.组合号;
                            可用仓位Row.证券代码 = 可用仓位1.证券代码;
                            可用仓位Row.证券名称 = 可用仓位1.证券名称;

                            this.Add可用仓位Row(可用仓位Row);
                        }
                    }
                    this.可用仓位RowChanging += 可用仓位R_可用仓位RowChanging;
                    this.可用仓位RowDeleting += 可用仓位R_可用仓位RowChanging;
                }
            }

            public bool Add(string 组合号, string 证券代码, string 证券名称, decimal 可用数量)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    if (this.FirstOrDefault(_ => _.组合号 == 组合号 && _.证券代码 == 证券代码) == null)
                    {
                        var row = this.New可用仓位Row();
                        row.总仓位 = 可用数量;
                        row.组合号 = 组合号;
                        row.证券代码 = 证券代码;
                        row.证券名称 = 证券名称;
            ...
            internal void AddOrUpdate(string 组合号, string 证券代码, string 证券名称, decimal 可用数量)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    var row = this.FirstOrDefault(_ => _.组合号 == 组合号 && _.证券代码 == 证券代码);
                    if (row == null)
                    {
                        row = this.New可用仓位Row();
                        row.总仓位 = 可用数量;
            ...
            public string Update(string 组合号, string 证券代码, string 证券名称, decimal 总仓位)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    var row = this.FirstOrDefault(_ => _.组合号 == 组合号 && _.证券代码 == 证券代码);
            ...
            public void Delete(string 组合号, string 证券代码)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    var row = this.FirstOrDefault(_ => _.组合号 == 组合号 && _.证券代码 == 证券代码);
                    this.Remove可用仓位Row(row);
                    Program.db.额度分配.DeleteLimit(组合号, 证券代码);
                }
            }
            ...
            public void DeleteAll()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    for (int i = this.Count - 1; i >= 0; i--)
                    {
                        this.Remove可用仓位Row(this.Rows[i] as 可用仓位Row);
                    }
                }
                Program.db.额度分配.DeleteAll();
            }

            public string LockPosition(string 组合号, string 证券代码, decimal 申请数量, string 缩写, string 交易员)
            {
                var group = Program.db.券商帐户.QueryQsAccount(组合号);
                if (group == null)
                {
                    return "额度锁定失败，未找到对应券商账户，请联系管理员!";
                }
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {

                    var lockItem = this.FirstOrDefault(_ => _.组合号 == 组合号 && _.证券代码 == 证券代码);
                    decimal lockNumber = Program.db.额度分配.Get额度分配(组合号, 证券代码).Sum(_ => _.交易额度);

                    额度分配Row limitExist = Program.db.额度分配.Get额度分配(交易员, 组合号, 证券代码);
                    if (limitExist != null)
                    {
                        lockNumber -= limitExist.交易额度;
                    }

                    if ((lockItem.总仓位 - lockNumber) >= 申请数量)
                    {

                        if (limitExist == null)
                        {
                            额度分配Row limitItem = Program.db.额度分配.New额度分配Row();
                            limitItem.交易员 = 交易员;
                            limitItem.组合号 = 组合号;
                            limitItem.证券代码 = 证券代码;
                            limitItem.市场 = CommonUtils.GetCodeMarket(证券代码);
                            limitItem.证券名称 = lockItem.证券名称;
                            limitItem.拼音缩写 = 缩写;
                            limitItem.交易额度 = 申请数量;
                            limitItem.手续费率 = group.手续费率;
                            limitItem.买模式 = group.买入方式;
                            limitItem.卖模式 = group.卖出方式;

                            Program.db.额度分配.Add额度分配Row(limitItem);
                            return "额度锁定添加成功";
                        }
                        else
                        {
                            //limitExist.交易额度 = 申请数量;
                            Program.db.额度分配.UpdateTradeLimit(交易员, 证券代码, 组合号, limitExist.市场, limitExist.证券名称, 缩写, (买模式)limitExist.买模式, (卖模式)limitExist.卖模式, 申请数量, group.手续费率);
                            return "额度锁定修改成功";
                        }
                    }
                    else
                    {
                        return string.Format("额度锁定异常, 请求数量{0}大于剩余仓位, 总仓位{1}, 已锁定额度{2}", 申请数量, lockItem.总仓位, lockNumber);
                    }
                }
            }

            public 可用仓位DataTable QueryAll()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    var dt = new 可用仓位DataTable();
                    foreach (var item in this)
                    {
                        dt.Add可用仓位Row(item.组合号, item.证券代码, item.证券名称, item.总仓位);
                    }
                    return dt;
                }
            }

            public DbDataSet.额度分配DataTable GetPositionLockDetail(string 组合号, string 证券代码)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    var position = this.FirstOrDefault(_ => _.组合号 == 组合号 && _.证券代码 == 证券代码);
                    if (position != null)
                    {
                        return Program.db.额度分配.Get额度分配(组合号, 证券代码);
                    }
                    else
                    {
                        return null;
            ...
            public int GetPositionLock(string 组合号, string 证券代码)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    var position = this.FirstOrDefault(_ => _.组合号 == 组合号 && _.证券代码 == 证券代码);
                    if (position != null)
                    {
                        return (int)Program.db.额度分配.Get额度分配(组合号, 证券代码).Sum(_ => _.交易额度);
                    }
                    else
                    {
                        return 0;
            ...
            public List<可用仓位Extend> GetPositonLockAll()
            {
                var name = OperationContext.Current.ServiceSecurityContext.PrimaryIdentity.Name;
                List<string> allowedGroups = GetAllowedGroups(name);

                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    var codes = this.Select(_=> _.证券代码).Distinct().ToArray();
                    StockInfo.StockInfoUtils.Init(codes);

                    List<可用仓位Extend> list = new List<可用仓位Extend>();
                    foreach (可用仓位Row item in this)
                    {
                        if (allowedGroups.Contains(item.组合号))
                        {
                            decimal lockedQty = Program.db.额度分配.Get额度分配(item.组合号, item.证券代码).Sum(_ => _.交易额度);
                            decimal priceYesterday = StockInfo.StockInfoUtils.GetPriceYesterday(item.证券代码);
                            list.Add(new 可用仓位Extend()
                            {
                                组合号 = item.组合号,
                                证券代码 = item.证券代码,
                                证券名称 = item.证券名称,
                                ...
                                已用数量 = lockedQty,
                                昨收 = priceYesterday,
                                剩余数量 = item.总仓位 - lockedQty,
                                剩余市值 = (item.总仓位 - lockedQty) * priceYesterday
                            });
                        }
                    }
                    return list;
            ...
            private List<string> GetAllowedGroups(string name)
            {
                var user = Program.db.平台用户.Get平台用户(name);
                bool notTrader = user.角色 != (int)角色.交易员;
                if (notTrader)
                {
                    return this.Select(_ => _.组合号).Distinct().ToList() ;
                }
                else
                {
                    return Program.db.组合号交易员关联.Where(_ => notTrader || _.交易员 == name).Select(_ => _.组合号).ToList();
                }
                
            }

            private void 可用仓位R_可用仓位RowChanging(object sender, 可用仓位RowChangeEvent e)
            {
                using (AASDbContext db = new AASDbContext())
                {
                    if (e.Action == DataRowAction.Add)
                    {
                        可用仓位 可用仓位1 = new 可用仓位();
                        可用仓位1.组合号 = e.Row.组合号;
                        可用仓位1.证券代码 = e.Row.证券代码;
                        可用仓位1.证券名称 = e.Row.证券名称;
                        可用仓位1.总仓位 = e.Row.总仓位;
                        db.可用仓位.Add(可用仓位1);
                        db.SaveChanges();
                    }
                    ...
                    {
                        可用仓位 row = db.可用仓位.Find(e.Row.组合号, e.Row.证券代码);
                        db.可用仓位.Remove(row);
                        db.SaveChanges();
                    }
            ...
            public struct 可用仓位Extend
            {
                ...
                public decimal 已用数量 { get; set; }

                public decimal 剩余数量 { get; set; }

                public decimal 昨收 { get; set; }

                public decimal 剩余市值 { get; set; }
            }
        }

        partial class 可用仓位Row
        {

        }

        partial class 组合号交易员关联DataTable
        {
            public ReaderWriterLockSlim readerWriterLockSlim = new ReaderWriterLockSlim();
            ...
            public void Load()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    this.PrimaryKey = new DataColumn[] {  this.Columns["组合号"], this.Columns["交易员"] };
                    using (AASDbContext db = new AASDbContext())
                    {
                        foreach (组合号交易员关联 组合号交易员关联1 in db.组合号交易员关联)
                        {
                            组合号交易员关联Row 关联Row = this.New组合号交易员关联Row();
                            
                            关联Row.组合号 = 组合号交易员关联1.组合号;
                            关联Row.交易员 = 组合号交易员关联1.交易员;
                            this.Add组合号交易员关联Row(关联Row);
                        }
                    }
                    this.组合号交易员关联RowChanging += 组合号交易员关联_RowChanging;
                    this.组合号交易员关联RowDeleting += 组合号交易员关联_RowChanging;
                }
            }

            private void 组合号交易员关联_RowChanging(object sender, 组合号交易员关联RowChangeEvent e)
            {
                using (AASDbContext db = new AASDbContext())
                {
                    if (e.Action == DataRowAction.Add)
                    {
                        组合号交易员关联 组合号交易员关联1 = new 组合号交易员关联();
                        组合号交易员关联1.组合号 = e.Row.组合号;
                        组合号交易员关联1.交易员 = e.Row.交易员;
                        
                        db.组合号交易员关联.Add(组合号交易员关联1);
                        db.SaveChanges();
                    }
                    else if (e.Action == DataRowAction.Delete)
                    {
                        组合号交易员关联 row = db.组合号交易员关联.Find(e.Row.组合号, e.Row.交易员);
                        db.组合号交易员关联.Remove(row);
                        db.SaveChanges();
                    }
                    else if (e.Action == DataRowAction.Change)
                    {
            ...
            public 组合号交易员关联DataTable Query(string 交易员)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    var dt = new 组合号交易员关联DataTable();
                    foreach (var item in this)
                    {
                        if (item.交易员 == 交易员)
                        {
                            dt.Add组合号交易员关联Row(item);
                        }
                    }
                    return dt;
                }
            ...
            public 组合号交易员关联DataTable QueryAll()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    var dt = this.Copy() as 组合号交易员关联DataTable;
                    return dt;
                }
            }

            public string Add(string 组合号, string 交易员)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Write))
                {
                    try
                    {
                        if (this.FirstOrDefault(_ => _.组合号 == 组合号 && _.交易员 == 交易员) == null)
                        {
                            var row = this.New组合号交易员关联Row();
                            row.组合号 = 组合号;
                            row.交易员 = 交易员;
                            this.Add组合号交易员关联Row(row);
                            return "1|添加成功";
                        }
                        else
                        {
                            return string.Format("0|添加失败，已存在交易员{0}配置的组合号{1}", 交易员, 组合号);
                        }
                    }
                    catch (Exception ex)
                    {
                        return string.Format("添加异常, 异常信息{0}", ex.Message);
                    }
                }
            }

            public void Delete(string 组合号, string 交易员)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    var rows = this.Where(_ => _.组合号 == 组合号 && _.交易员 == 交易员).ToArray();
                    foreach (var item in rows)
                    {
                        this.Remove组合号交易员关联Row(item);
                    }
                }
            ...
            public void DeleteByAccount(string Account)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    var rows = this.Where(_ => _.组合号 == Account).ToArray();
                    foreach (var item in rows)
                    {
                        this.Remove组合号交易员关联Row(item);
                    }
            ...
            public void DeleteByTrader(string trader)
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    var rows = this.Where(_ => _.交易员 == trader).ToArray();
                    foreach (var item in rows)
                    {
                        this.Remove组合号交易员关联Row(item);
                    }
