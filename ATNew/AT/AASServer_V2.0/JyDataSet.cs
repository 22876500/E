            ...
            private void SendToTrader()
            {
                System.Threading.Tasks.Task.Run(() => 
                {
                    foreach (var item in Program.ClientUserName)
                    {
                        try
                        {
                            if (item.Value == this.交易员)
                            {
                                item.Key.委托Change(this.委托编号, this.状态说明, this.成交数量, this.成交价格, this.撤单数量);
                            }
                        }
                        catch (Exception ex)
                        {
                            Program.logger.LogInfoDetail("SendToTrader Exception:" + ex.Message);
                        }
                    }
                });
            }
