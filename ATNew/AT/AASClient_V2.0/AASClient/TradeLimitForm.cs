...
namespace AASClient
{
    public partial class TradeLimitForm : Form
    {
        ...
        private void TradeLimitForm_Load(object sender, EventArgs e)
        {
            ...
            this.dataGridView额度分配.Columns["组合号"].Visible = true;
            ...
            this.dataGridView额度分配.Columns.Insert(2, new DataGridViewTextBoxColumn() { HeaderText = "可卖股数", Name = "可卖股数", Width = 60 });
            //this.dataGridView额度分配.Columns.Add("可卖股数", "可卖股数");
            this.dataGridView额度分配.Columns.Add("已卖股数", "已卖股数");
            this.dataGridView额度分配.Columns["证券代码"].HeaderText = "证券代码";
            this.dataGridView额度分配.Columns["证券名称"].HeaderText = "证券名称";
            this.dataGridView额度分配.Columns["交易额度"].HeaderText = "交易额度";
            ...
            this.dataGridView额度分配.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells;
        ...
        private void dataGridView额度分配_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            var item = (dataGridView额度分配.Rows[e.RowIndex].DataBoundItem as DataRowView).Row as AASServiceReference.DbDataSet.额度分配Row;
            if (item != null)
            {
                var parent = this.Owner as TradeMainForm;
                if (parent != null)
                {
                    parent.AddHqForm(item.证券代码);
                }
            }
        }
    }
