    ...
    public partial class AddQSAccountForm : Form
    {
        ...
        private string GetTestResult(string[] JyServerInfo)
        {
            try
            {
                ...
                string Ret = Program.AASServiceClient.TestEx(this.textBox帐户名称.Text, JyServerInfo[1], short.Parse(JyServerInfo[2]), this.textBox版本号.Text, (short)this.numericUpDown营业部代码.Value, this.textBox登录帐号.Text, this.textBox交易帐号.Text, this.textBox交易密码.Text, this.textBox通讯密码.Text);
                return Ret;
            }
            catch (Exception ex)
            {
        ...
        private void button添加帐号_Click(object sender, EventArgs e)
        {
            ...
            bool enable = checkBox启用.Checked;
            string accountName = textBox帐户名称.Text;
            string account = comboBox券商.Text;
            string accountType = comboBox类型.Text;
            string serverAddr = comboBox交易服务器.Text;

            Program.AASServiceClient.AddQSAccount(enable, accountName, account, accountType, serverAddr,
                this.textBox版本号.Text, (short)this.numericUpDown营业部代码.Value, this.textBox登录帐号.Text, this.textBox交易帐号.Text, 
                this.textBox交易密码.Text, this.textBox通讯密码.Text, this.textBox上海股东代码.Text, 
                this.textBox深圳股东代码.Text, (int)this.numericUpDown查询时间间隔.Value, 0, comboBox买入方式.SelectedIndex, comboBox卖出方式.SelectedIndex);
