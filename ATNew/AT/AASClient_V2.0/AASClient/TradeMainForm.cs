...
namespace AASClient
{
    public partial class TradeMainForm : Form
    {
        ...
        public TradeMainForm()
        {
            ...
            this.hqForm = new List<HqForm>(this.hqFormCount);
        ...
        private void backgroundWorker报价_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            switch (e.ProgressPercentage)
            {
                ...
                case 2:
                    ...
                    Tool.RefreshDrcjDataTable(Program.serverDb.额度分配, TradeLimitDataTable, new string[] { "证券代码", "组合号" });
                        ...
                        if (TDFData.DataSourceConfig.IsUseTDXData)
                        {
                            ...
                            else
                            {
                                ...
                                this.Invoke(new Action(() => {
                                    for (int i = 0; i < this.hqForm.Count; i++)
                                    {
        ...
        public void AddHqForm(string code)
        {
            for (int i = 0; i < hqForm.Count; i++)
            {
                if (hqForm[i].Zqdm == code)
                {
                    if (!hqForm[i].Visible)
                    {
                        hqForm[i].Visible = true;
                        hqForm[i].Show(this.dockPanel1);
                    }
                    hqForm[i].Focus();
                    return;
                }
            ...
            Program.accountDataSet.参数.SetParaValue("证券代码" + this.hqForm.Count, code);
            var newHq = new HqForm(this.hqForm.Count, this);
            this.hqForm.Add(newHq);
            newHq.Show(this.dockPanel1);

            Program.accountDataSet.参数.SetParaValue("报价窗口数目", this.hqForm.Count + "");
            this.hqFormCount = this.hqForm.Count;
        }

        private void 共享额度ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var shareLimitWin = new ShareLimitListForm() { StartPosition = FormStartPosition.CenterParent };
            shareLimitWin.Show(this);
        }

        private void 可用仓位ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var win = new StockPosition.PositionList();
            win.Show(this);
        }

        private void 新建报价窗口ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            AddHqForm("000001");
        }
    }
