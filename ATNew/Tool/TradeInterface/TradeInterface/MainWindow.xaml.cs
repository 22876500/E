...
namespace TradeInterface
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private static Dictionary<string, decimal> CalculateTotalBuyCount(DataTable dt1, string codeColumnName1)
        {
            ...
            bool hasBsFlag = dt1.Columns.Contains("买卖标志");
            foreach (DataRow dr in dt1.Rows)
            {
                string code = dr[codeColumnName1] + "";
                decimal count = CommonUtils.GetDecimal(dr["成交数量"]);
                bool isCancel = hasBsFlag && (dr["买卖标志"] + "").Contains("撤");
                if (CommonUtils.IsCode(code) && !isCancel && count > 0)
                {
