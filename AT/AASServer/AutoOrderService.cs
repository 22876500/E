...
namespace AASServer
{
    public class AutoOrderService
    {
        ...
        {
            #region 配置信息初始化
            var port = CommonUtils.GetConfig("ZeroMQPubPort");
            if (!string.IsNullOrEmpty(port) && Regex.IsMatch(port, "^[0-9]+$"))
            {
                this.PubPort = int.Parse(port);
            }
            var repPort = CommonUtils.GetConfig("ZeroMQRepPort");
        ...
        private static void QueryOrderMessageInit(ZMessage zMsgReply, string traderName)
        {
            ...
            {
                var lstOrder = new List<AASServer.订单>();
        ...
        public string CancelOrder(string UserName, string 证券代码, string 委托编号)
        {
            ...
            try
            {
                ...
                if (ErrInfo == string.Empty)
                {
                    ...
                    //if (wtGroup != null && wtGroup.成交数量 < wtGroup.委托数量)
                    //{
                    //    wtGroup.撤单数量 = wtGroup.委托数量 - wtGroup.成交数量;
                    //}
                    //if (wtUser != null && wtUser.成交数量 < wtUser.委托数量)
                    //{
                    //    wtUser.撤单数量 = wtUser.委托数量 - wtUser.成交数量;
                    //}
