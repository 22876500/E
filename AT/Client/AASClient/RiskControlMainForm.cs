    ...
    public partial class RiskControlMainForm : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            Task.Run(() => {
                ...
                JyDataSet.委托DataTable wtCopy = null;
                lock (Program.jyDataSet.委托)
                {
                    wtCopy = Program.jyDataSet.委托.Copy() as JyDataSet.委托DataTable;
                }
                当日委托交易费用 = wtCopy.Where(r => r.成交数量 > 0).Sum(_ => _.Get交易费用());

                var fc = new FlushClient(() =>
                {
        ...
        private void RefreshDataGridView(decimal 当日委托交易费用, decimal 毛利, JyDataSet.委托DataTable wtCopy)
        {
            ...
            {
                //Program.logger.LogRunning("RefreshDataGridView Exception, Info {0}", ex.Message);
            }
