...
namespace AASServer
{
    ...
    public partial class DbDataSet
    {
        ...
        partial class 券商帐户DataTable
        {
            ...
            public void CancelOrder(string 组合号, byte Market, string hth, out string Result, out string ErrInfo)
            {
                Result = string.Empty;
                ErrInfo = string.Empty;
                ...
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    ...
                    券商帐户1.CancelOrder(Market, hth, out Result, out ErrInfo);
                }
            }
        ...
        partial class 券商帐户Row
        {
            ...
            bool IsCATSAccount 
            {
                get
                {
                    if (catsGroups == null)
                    {
                        catsGroups = CommonUtils.GetConfig("CATSGroups");
                    }
                    return !string.IsNullOrEmpty(catsGroups) && catsGroups.Contains(this.名称);
                }
            }
            ...
            void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
            {
                while (!this.backgroundWorker1.CancellationPending)
                {
                    if (this.Safe启用)
                    {
                        if (DateTime.Parse(Program.appConfig.GetValue("开始查询时间", "8:15")) <= DateTime.Now && DateTime.Now <= DateTime.Parse(Program.appConfig.GetValue("结束查询时间", "15:30")))
                        {
                            if (!CommonUtils.ExistsGroup(this.名称) && CommonUtils.GetConfig("LocalGroup") != this.名称)
                            {
                                this.backgroundWorker1.ReportProgress(0, "未配置组合号对应接口地址!");
                                Thread.Sleep(5000);
                            }
                            else
                            {
                    ...
                    if (CommonUtils.ExistsGroup(this.名称))
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Program.logger.LogInfo("连接异常：", ex);
                                }
                                catch (Exception)
                                {
                                    //return "登录失败:" + ex.Message;
                                }
                                return ClientID > -1 ? "登录成功" : "登录失败, 请检查终端是否已启动";
                            }
                        }
                    }
                    else if (CommonUtils.GetConfig("LocalGroup") == this.名称)
                    {
            ...
            static Regex regexOrderID = new Regex("(?<=[0-9]{2}:[0-9]{2}:[0-9]{2}\t)[0-9]+");
            ...
            public string QueryData()
            {
                ...
                var client = CommonUtils.GetGroupClient(this.名称);
                ServiceReference.GroupServiceQueryDataObj response = null;
                ...
                if (this._isMultyThread == true)
                {
                    ...
                    {
                        //1.更新最新订单号
                        var arrID = Program.db.已发委托.Where(_ => _.组合号 == this.名称 && _.日期 == DateTime.Today).Select(_ => _.委托编号).ToList();
                        if (arrID.Count > 0)
                        {
                            client.UpdateOrderIDList(CommonUtils.Mac, this.名称, arrID.ToArray());
                            //Program.logger.LogInfoDetail(idReturn.Length.ToString());
                        }
                ...
                {
                    #region 委托及成交更新 普通券商
                    response = client.QueryDataAuto(this.名称);
                    if (response.SearchTradeResult != null)
                    {
                        查到的成交Result = Tool.ChangeDataStringToTable(response.SearchTradeResult);
                    }
                    查到的成交ErrInfo = response.SearchTradeErrInfo;

                    if (!string.IsNullOrEmpty(response.SearchOperatorResult))
                    {
                        查到的委托Result = Tool.ChangeDataStringToTable(response.SearchOperatorResult);    
                    }

                    查到的委托ErrInfo = response.SearchOperatorError;
            ...
            private string CATSQueryData()
            {
                ...
                {
                    List<CATSEntity.StandardOrderEntity> list;
                    while (queueStandardOrder.TryDequeue(out list))
                    {
                        RefreshCatsOrder(list);
                    }
                }
            ...
            ConcurrentQueue<List<CATSEntity.StandardOrderEntity>> queueStandardOrder;
            ...
            private void RefreshCatsOrder(List<CATSEntity.StandardOrderEntity> obj)
            {
                if (obj.Count > 0)
                {
                    var specItems = obj.Where(_ => _.Account.Trim() != this.交易帐号).ToList();
                    if (specItems != null && specItems.Count > 0)
                    {
                        Program.logger.LogInfoDetail("当前组合号账户为{0}, 交易帐号异常项为{1}", this.交易帐号, specItems.ToJson());
                    }
                    obj = obj.Where(_ => _.Account.Trim() == this.交易帐号).ToList();
                    List<JyDataSet.委托Row> rowNeedAdd = new List<JyDataSet.委托Row>();
                    Program.logger.LogInfoDetail("过滤后数据总量：{0}", obj.Count);
                    foreach (var item in obj)
                    {

                        try
                        {
                        var wtItem = 帐户委托DataTable.FirstOrDefault(_ => (_.委托编号 == item.OrderNo || _.委托编号 == item.ClientID));
                        if (wtItem == null)
                        {
                                var dbItem = Program.db.已发委托.FirstOrDefault(_ => _.日期 == DateTime.Today && _.组合号 == this.名称 && _.委托编号 == item.ClientID || _.委托编号 == item.OrderNo);
                                var cacheItem = CommonUtils.OrderCacheQueue.FirstOrDefault(_ => _.GroupName == this.名称 && (_.ClientGUID == item.ClientID));
                            JyDataSet.委托Row wtAdd = null;
                            if (cacheItem != null)
                            {
                                AddToDB(item, dbItem, cacheItem);

                                wtAdd = AddToCache(item, cacheItem);
                            }
                            else if (dbItem != null)
                            {
                                wtAdd = 帐户委托DataTable.New委托Row();

                                wtAdd.市场代码 = dbItem.市场代码;
                                wtAdd.买卖方向 = dbItem.买卖方向;
                                wtAdd.交易员 = dbItem.交易员;
                                wtAdd.组合号 = dbItem.组合号;
                                wtAdd.证券名称 = dbItem.证券名称;
                                wtAdd.状态说明 = item.OrderStatus;
                                wtAdd.证券代码 = item.StockCode.Trim();
                                wtAdd.委托数量 = item.OrderQty;
                                wtAdd.委托时间 = item.OrderTime.ToString("HH:mm:ss");
                                wtAdd.委托价格 = Math.Round(item.OrderPrice, 5);
                                wtAdd.委托编号 = item.OrderNo;
                                wtAdd.撤单数量 = item.CancelQty;
                                wtAdd.成交价格 = Math.Round(item.FilledPrice, 5);
                                wtAdd.成交数量 = item.FilledQty;
                                帐户委托DataTable.Add委托Row(wtAdd);
                            }

                                if (wtAdd.成交数量 > 0)
                            {
                                AddCjFromWt(item, wtAdd);
                            }
                        }
                        else
                        {
                            UpdateWtCache(item, wtItem);
                        }
                    }
                        catch (Exception ex)
                        {
                            Program.logger.LogInfoDetail("RefreshCatsOrder Exception, Message {0}", ex.Message);
                }
            }
                }
            }

            private void UpdateWtCache(CATSEntity.StandardOrderEntity item, JyDataSet.委托Row wtItem)
            {
                wtItem.撤单数量 = item.CancelQty;
                wtItem.成交价格 = item.FilledPrice;
                wtItem.成交数量 = item.FilledQty;
                wtItem.状态说明 = item.OrderStatus;

                if (wtItem.成交数量 > 0)
                {
                    AddCjFromWt(item, wtItem);
                }
            }

            private void AddCjFromWt(CATSEntity.StandardOrderEntity item, JyDataSet.委托Row wtItem)
            {
                var cjOldRow = 帐户成交DataTable.FirstOrDefault(_ => _.委托编号 == wtItem.委托编号);
                if (cjOldRow == null)
                {
                    var cjNewRow = 帐户成交DataTable.New成交Row();
                    cjNewRow.组合号 = wtItem.组合号;
                    cjNewRow.证券代码 = wtItem.证券代码;
                    cjNewRow.证券名称 = wtItem.证券名称;
                    cjNewRow.委托编号 = wtItem.委托编号;
                    cjNewRow.买卖方向 = wtItem.买卖方向;
                    cjNewRow.市场代码 = wtItem.市场代码;
                    cjNewRow.成交编号 = 帐户成交DataTable.Rows.Count.ToString();
                    
                    cjNewRow.成交时间 = wtItem.委托时间;
                    cjNewRow.成交价格 = wtItem.成交价格;
                    cjNewRow.成交数量 = wtItem.成交数量;
                    cjNewRow.成交金额 = wtItem.成交价格 * wtItem.成交数量;
                    cjNewRow.交易员 = wtItem.交易员;
                    帐户成交DataTable.Add成交Row(cjNewRow);
                }
                else
                {
                    cjOldRow.成交数量 = item.FilledQty;
                    cjOldRow.成交价格 = item.FilledPrice;
                    cjOldRow.成交金额 = item.FilledPrice * item.FilledQty;
                }
            }

            private JyDataSet.委托Row AddToCache(CATSEntity.StandardOrderEntity item, OrderCacheEntity cacheItem)
            {
                var newWt = 帐户委托DataTable.New委托Row();

                newWt.市场代码 = cacheItem.Market;
                newWt.买卖方向 = cacheItem.Category;
                newWt.交易员 = cacheItem.Trader;
                newWt.组合号 = cacheItem.GroupName;
                newWt.状态说明 = item.OrderStatus;
                newWt.证券名称 = cacheItem.ZqName;
                newWt.证券代码 = item.StockCode.Trim();
                newWt.委托数量 = item.OrderQty;
                newWt.委托时间 = item.OrderTime.ToString("HH:mm:ss");
                newWt.委托价格 = Math.Round(item.OrderPrice, 5);
                newWt.委托编号 = item.OrderNo;
                newWt.撤单数量 = item.CancelQty;
                newWt.成交价格 = Math.Round(item.FilledPrice, 5);
                newWt.成交数量 = item.FilledQty;
                帐户委托DataTable.Add委托Row(newWt);
                return newWt;
            }

            private void AddToDB(CATSEntity.StandardOrderEntity item, 已发委托Row dbItem, OrderCacheEntity cacheItem)
            {
                if (dbItem == null)
                {
                    Program.db.已发委托.Add(DateTime.Today, this.名称, item.OrderNo, cacheItem.Trader, "委托成功",
                        cacheItem.Market, cacheItem.Zqdm.Trim(), cacheItem.ZqName, cacheItem.Category, 0m, 0m, cacheItem.Price, (decimal)cacheItem.Quantity, 0m);

                    string Msg = cacheItem.Sender == cacheItem.Trader ? "下单成功" : string.Format("风控员{0}下单成功", cacheItem.Trader);

                    Program.db.交易日志.Add(DateTime.Today, DateTime.Now.ToString("HH:mm:ss"), cacheItem.Trader,
                        this.名称, cacheItem.Zqdm.Trim(), cacheItem.ZqName, cacheItem.OrderID, cacheItem.Category, cacheItem.Quantity, cacheItem.Price, Msg);
                }
            ...
            {
                if (查到的委托DataTable.Columns.Contains("撤单标志"))
                {
                    for (int i = 查到的委托DataTable.Rows.Count - 1; i >= 0; i--)
                    {
                        if (查到的委托DataTable.Rows[i]["撤单标志"] as string == "1")
                        {
                            查到的委托DataTable.Rows.RemoveAt(i);
                        }
                    }
                }
                ...
                //                 && 查到的委托DataTable.Columns.Contains("委托时间")
            ...
            public void SendOrder(int Category, byte Market, string Zqdm, decimal Price, decimal Quantity, OrderCacheEntity orderCacheObj, out string Result, out string ErrInfo, out bool hasOrderNo)
            {
                ...
                try
                {
                    //if (!CommonUtils.ExistsGroup(this.名称))
                    //{
                    //    this.SendOrderLocal(Category, Market, Zqdm, Price, Quantity, out Result, out ErrInfo);
                    //    return;
                    //}
                    if (this.ClientID == -1)
                    {
                    ...
                    下单response = client1.SendOrder(this.名称, Category, 0, 股东代码, Zqdm, (float)Price, (int)Quantity);
                    client1.Close();
                    ...
                    {
                        #region Check Lost Order
                        if (ErrInfo.IndexOf("尝试其他交易服务器") != -1 || ErrInfo.IndexOf("超时") != -1)
                        {
                            DataTable 下单前委托DataTable = Tool.ChangeDataStringToTable(下单response.Other);
                            JyDataSet.委托DataTable 下单前规范的委托DataTable = this.Get规范委托(下单前委托DataTable);

                            DataTable 下单后委托DataTable = null;
                            string 下单后ErrInfo = string.Empty;

                            Thread.Sleep(100);
                            var client2 = CommonUtils.GetGroupClient(this.名称);
                            var response = client2.QueryData(this.名称, 2);
                            client2.Close();

                            下单后ErrInfo = response.Error;
                            if (下单后ErrInfo == string.Empty)
                            {
                                下单后委托DataTable = Tool.ChangeDataStringToTable(response.Result);
                            }



                            JyDataSet.委托DataTable 下单后规范的委托DataTable = this.Get规范委托(下单后委托DataTable);

                            for (int i = 下单前规范的委托DataTable.Rows.Count; i < 下单后规范的委托DataTable.Rows.Count; i++)
                            {
                                //Category 0 2 4 买        1 3 5卖
                                if (Category % 2 == 下单后规范的委托DataTable[i].买卖方向 &&
                                    Market == 下单后规范的委托DataTable[i].市场代码 &&
                                    Zqdm == 下单后规范的委托DataTable[i].证券代码 as string &&
                                    Price == 下单后规范的委托DataTable[i].委托价格 &&
                                    Quantity == 下单后规范的委托DataTable[i].委托数量
                                    )
                                {
                                    ErrInfo = string.Empty;
                                    Result = 下单后规范的委托DataTable[i].委托编号;
                                    return;
                                }
                            }
                            Result = string.Empty;
                ...
                catch (Exception ex)
                {
                    ...
                    Program.logger.LogInfo(string.Format("SendOrder下单异常：{0}", ex.StackTrace));
                }

            }

            public void SendOrderLocal(int Category, byte Market, string Zqdm, decimal Price, decimal Quantity, out string Result, out string ErrInfo)
            {
                lock (this.SendOrderObject)
                {
                    Result = string.Empty;
                    ...
                    DataTable 下单前委托DataTable;
                    string 下单前ErrInfo;
                    Program.logger.LogInfoDetail("1.{0}下单前自动查询开始：时间{1}", this.名称, DateTime.Now.ToString("HH:mm:ss:fff"));
                    while (true)
                    {
                        #region 下单前查一下委托
                        TdxApi.QueryData(this.ClientID, 2, Result1, ErrInfo1);
                        下单前委托DataTable = Tool.ChangeDataStringToTable(Result1.ToString());
                        下单前ErrInfo = ErrInfo1.ToString();
                        if (下单前ErrInfo == string.Empty)
                        {
                            break;
                        }
                    ...
                    JyDataSet.委托DataTable 下单前规范的委托DataTable = this.Get规范委托(下单前委托DataTable);
                    Program.logger.LogInfoDetail("2.{0}下单前自动查询结束，开始下单 {1}", this.名称, DateTime.Now.ToString("HH:mm:ss:fff"));
                    TdxApi.SendOrder(this.ClientID, Category, 0, 股东代码, Zqdm, (float)Price, (int)Quantity, Result1, ErrInfo1);
                    Program.logger.LogInfoDetail("3.{0}下单结束 {1}", this.名称, DateTime.Now.ToString("HH:mm:ss:fff"));
                    ErrInfo = ErrInfo1.ToString();

                    if (ErrInfo == string.Empty)
                    {
                        DataTable DataTable1 = Tool.ChangeDataStringToTable(Result1.ToString());
                    ...
                    else
                    {
                        ...
                        if (ErrInfo.IndexOf("尝试其他交易服务器") != -1 || ErrInfo.IndexOf("超时") != -1)
                        {
                            Program.logger.LogInfoDetail("4.{0}下单结束错误信息不为空，下单成功验证开始 {1}", this.名称, DateTime.Now.ToString("HH:mm:ss:fff"));
                            DataTable 下单后委托DataTable;
                            string 下单后ErrInfo;
                            while (true)
                            {
                                #region 下单后查一下委托
                                TdxApi.QueryData(this.ClientID, 2, Result1, ErrInfo1);
                                下单后委托DataTable = Tool.ChangeDataStringToTable(Result1.ToString());
                                下单后ErrInfo = ErrInfo1.ToString();
                                if (下单后ErrInfo == string.Empty)
                                {
                                    break;
                                }
                            ...
                            Program.logger.LogInfoDetail("4.{0}下单结束错误信息不为空，下单成功验证结束 {1}", this.名称, DateTime.Now.ToString("HH:mm:ss:fff"));
                            Result = string.Empty;
                            return;
                        }
                        else
            ...
            public void SendOrderHK(int Category, byte Market, string Zqdm, decimal Price, decimal Quantity, out string Result, out string ErrInfo)
            {
                Result = string.Empty;
                ErrInfo = string.Empty;
                ...
                if (!CommonUtils.ExistsGroup(this.名称))
                {
                    SendOrderHKLocal(Category, Market, Zqdm, Price, Quantity, out Result, out ErrInfo);
                    return;
                }
            ...
            public void SendOrderHKLocal(int Category, byte Market, string Zqdm, decimal Price, decimal Quantity, out string Result, out string ErrInfo)
            {
                lock (this.SendOrderObject)
                {
                    Result = string.Empty;
                    ...
                    {
                        #region 下单前查一下委托
                        TdxApi.QueryHKData(this.ClientID, 2, Result1, ErrInfo1);
                        下单前委托DataTable = Tool.ChangeDataStringToTable(Result1.ToString());
                        下单前ErrInfo = ErrInfo1.ToString();
                        if (下单前ErrInfo == string.Empty)
                        {
                    ...
                    JyDataSet.委托DataTable 下单前规范的委托DataTable = this.Get规范委托(下单前委托DataTable);
                    Program.logger.LogInfoDetail("2.{0}下单前自动查询结束，开始下单 {1}", this.名称, DateTime.Now.ToString("HH:mm:ss:fff"));
                    TdxApi.SendHKOrder(this.ClientID, Category, 2, 0, 股东代码, Zqdm, Price.ToString(), Quantity.ToString(), Result1, ErrInfo1);
                    Program.logger.LogInfoDetail("3.{0}下单结束 {1}", this.名称, DateTime.Now.ToString("HH:mm:ss:fff"));
                    ErrInfo = ErrInfo1.ToString();
            ...
            string newestCancelHth = string.Empty;


            public void CancelOrder(byte Market, string hth, out string Result, out string ErrInfo)
            {
                if (this.IsCATSAccount)
                {
                ...
                Result = string.Empty;
                ErrInfo = string.Empty;
                //var order = Program.db.已发委托.FirstOrDefault(_ => _.日期 == DateTime.Today && _.组合号 == this.名称 && _.委托编号 == hth);
                //if (order != null && order.证券代码.Length == 5)
                //{
                //    CancelOrderHK(Market, hth, out Result, out ErrInfo);
                //    return;
                //}
                if (!CommonUtils.ExistsGroup(this.名称))
                {
                    //this.CancelOrderLocal( Market,hth, out Result, out ErrInfo);
            ...
            public void CancelOrderLocal(byte Market, string hth, out string Result, out string ErrInfo)
            {
                Result = string.Empty;
                ErrInfo = string.Empty;
            ...
            public void CancelOrderHK(byte Market, string hth, out string Result, out string ErrInfo)
            {
                Result = string.Empty;
                ErrInfo = string.Empty;
                StringBuilder Result1 = new StringBuilder(1024 * 1024);
                ...
                else if (!CommonUtils.ExistsGroup(this.名称))
                {
                    TdxApi.CancelHKOrder(this.ClientID, hth, Result1, ErrInfo1);
                ...
                {
                    //Result = "GroupClient未提供此服务！";
                    var client = CommonUtils.GetGroupClient(this.名称);
                    var cancelResult = client.CancelOrderHK(this.名称, hth);
                    Result = cancelResult.Result;
                    ErrInfo = cancelResult.Error;
                    if (ErrInfo == string.Empty)
                    {
                        Result = "撤单成功";
                    }
            ...
            private void QueryPubStockInfo(out string Result, out string ErrInfo)
            {
                StringBuilder Result1 = new StringBuilder(1024 * 1024);
                StringBuilder ErrInfo1 = new StringBuilder(256);
                if (!CommonUtils.ExistsGroup(this.名称))
                {
                    TdxApi.QueryData(this.ClientID, 8, Result1, ErrInfo1);
                }
                else
                {
                    var client = CommonUtils.GetGroupClient(this.名称);
                    var queryResult = client.QueryData(this.名称, 8);
                    Result1.Append(queryResult.Result);
                    ErrInfo1.Append(queryResult.Error);
                }
                ErrInfo = ErrInfo1.ToString();
            ...
            public AASServer.DbDataSet.额度分配DataTable QueryTradeLimit()
            {
                using (ReadWriteLock ReadWriteLock1 = new ReadWriteLock(this.readerWriterLockSlim, ReadWriteMode.Read))
                {
                    ...
                    else
                    {
                        foreach (AASServer.DbDataSet.额度分配Row 额度分配Row1 in this)
                        {
                            AASServer.DbDataSet.平台用户Row 交易员Row = Program.db.平台用户.Get平台用户(额度分配Row1.交易员);
                            if (交易员Row.分组 == 平台用户Row.分组)
                            {
                        额度分配DataTable1.ImportRow(额度分配Row1);
                            }
