        ...
        public static IWorkbook RenderToExcel<T>(List<T> list, string[] Members, string fileName)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                for (int j = 0; j < Members.Length; j++)
                {
                    ...
                    {
                        CommonUtils.Log("ExcelUtils.RenderToExcel<T>(List<T> list, string[] Members, string fileName) Exception: " + ex.Message);
                    }
        ...
        private static int HeaderIndexFix(DataTable dt, ISheet sheet, int headerRowIndex)
        {
            if (headerRowIndex >= 0)
            {
                ...
                for (int i = 0; i < headerRow.LastCellNum; i++)
                {
                    ...
                    dt.Columns.Add(string.IsNullOrEmpty(header) ? i.ToString() : header.ToString());
                }
            }
            else
            {
